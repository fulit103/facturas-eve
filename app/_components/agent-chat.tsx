...
export function AgentChat({
  ...
  const handleSubmit = async (message: PromptInputMessage) => {
    ...
    for (const file of message.files) {
      const mediaType = inferInvoiceMediaType(file.filename ?? "archivo", file.mediaType);
      if (!mediaType) {
        setCancellationError("Solo se aceptan facturas en PDF, JPG o PNG.");
        return;
      }
  ...
  const composer = (
    <PromptInput
      ...
      onError={(error) => {
        setCancellationError(
          error.code === "accept"
            ? "Solo se aceptan facturas en PDF, JPG o PNG."
    ...
    >
      ...
      <PromptInputTextarea
        ...
        placeholder="Escribí un mensaje o adjuntá una factura…"
      />
      <PromptInputFooter>
        <PromptInputTools>
          <PromptInputActionMenu>
    ...
    <PromptInputHeader>
      {attachments.files.map((file) => (
        ...
        >
          ...
          <span className="max-w-[14rem] truncate">{file.filename ?? "Factura"}</span>
          <button
    ...
    <Message className="max-w-full" from="assistant">
      <MessageContent>
        ...
        >
          ...
          <div>
            <p className="font-medium">La solicitud falló</p>
      ...
      <div className="relative mx-auto flex h-full w-full max-w-3xl items-center justify-center bg-background px-24">
        ...
        {canStartNewChat ? (
          ...
          >
            ...
            <span className="hidden font-normal text-sm sm:inline">Nueva conversación</span>
          </Button>
        ) : null}
      </div>
    </header>
