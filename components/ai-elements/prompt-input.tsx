    ...
    setAttachmentFiles((prev) => [
      ...
      ...incoming.map((file) => {
        ...
        return {
          filename: file.name,
          id: nanoid(),
          mediaType: file.type || extensionToMime[extension] || file.type,
          type: "file" as const,
...
export const PromptInput = ({
  ...
  const matchesAccept = useCallback(
    (f: File) => {
      ...
      const effectiveType = f.type || extensionToMime[extension] || "";

      return patterns.some((pattern) => {
        if (pattern.startsWith(".")) {
          return extension === pattern.slice(1).toLowerCase();
        }
        if (pattern.endsWith("/*")) {
          const prefix = pattern.slice(0, -1);
          return effectiveType.startsWith(prefix);
        }
